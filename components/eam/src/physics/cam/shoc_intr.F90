...
! Broadcast namelist variables
  ...
  !------------------------------------------------------------------- !  
    ...
    use ppgrid,         only: pver, pverp, pcols
    use constituents,   only: cnst_get_ind, cnst_get_name
...
!print *,"shoc_intr_00 ",i,k,inv_exner(i,k),rvm(i,k),rcm(i,k),rtm(i,k),um(i,k),vm(i,k),thlm(i,k),thv(i,k),tke_zt(i,k),cloud_frac(i,k)
...
!print *,"shoc_intr_01 ",i,k,dz_g(i,k),zt_g(i,k),rrho(i,k),wm_zt(i,k),shoc_s(i,k)
     enddo
   enddo
...
!print *,"shoc_intr_02 ",i,k,zi_g(i,k)
     enddo
   enddo
...
!print *,"shoc_intr_03 ",i,wpthlp_sfc(i),wprtp_sfc(i),upwp_sfc(i),vpwp_sfc(i)
   enddo               
   
   !  Do the same for tracers 
   icnt=0
   do ixind=1,pcnst
print *,"tracer names: ixind ",ixind, " ;name ", cnst_get_name(ixind) 
     if (lq(ixind))  then 
       icnt=icnt+1
       do k=1,pver
         do i=1,ncol
           edsclr_in(i,k,icnt) = state1%q(i,k,ixind)
!print *,"shoc_intr_04 ",icnt,i,k,edsclr_in(i,k,icnt)
         enddo
       enddo
     end if
   enddo    
   ...
   do k=1,pver
     do i=1,ncol
print *,"shoc_intr_05 ",i,k,host_dx_in(i),host_dy_in(i),thv(i,k),zt_g(i,k),zi_g(i,k),state%pmid(i,k),state%pint(i,k),state1%pdel(i,k)
     enddo
   enddo

   do k=1,pver
     do i=1,ncol
print *,"shoc_intr_06 ",i,k,wpthlp_sfc(i),wprtp_sfc(i),upwp_sfc(i),vpwp_sfc(i),wm_zt(i,k),inv_exner(i,k),tke_zt(i,k),thlm(i,k),rtm(i,k)
     enddo
   enddo

   do k=1,pver
     do i=1,ncol
print *,"shoc_intr_07 ",i,k,um(i,k),vm(i,k),wthv(i,k),tkh(i,k),tk(i,k),rcm(i,k),cloud_frac(i,k)
     enddo
   enddo
   ...
   do k=1,pver
     do i=1,ncol
       do j = 1, edsclr_dim
print *,"shoc_intr_08 ",i,k,j,edsclr_in(i,k,j)
       enddo
     enddo
   enddo
#endif
